...
@section('script')
        ...
        for(var i = 0; i < data.length; i++){
          var requestEntryHistoryData = '<li class="completed">';
              ...
              if(data[i].request_file){
                ...
                requestEntryHistoryData += '<button id="'+i+'" data-file="'+data[i].request_file.file_upload+'" style="text-align: center" type="button" class="btn btn-sm btn-success px-2 btn-requestEntry_ModalFilePreview" data-toggle="modal" data-target="#filePreviewModal">'+data[i].request_file.file_upload+'</button>';
              }
  ...
  <script>
    ...
    $('.btn-request_legalEdit').on('click', function(e){
      $(".btn-requestLegalEntrySummaryInsert").css({display: "none"});
      $(".btn-requestLegalEntrySummaryEdit").css({display: "block"});
    ...
    $('.btn-requestLegalEntryInsert').on('click', function(e){
      $(".btn-requestLegalEntrySummaryInsert").css({display: "block"});
      $(".btn-requestLegalEntrySummaryEdit").css({display: "none"});
    }); 
